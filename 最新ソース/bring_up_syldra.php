...
$eggPoint = $query->fetch();
	if($eggPoint[0]>=100){
		$pdo = new PDO('mysql:dbname=LAA0535115-dbname;host=ホスト名', 'ユーザー名', 'パスワード');
$pdo->query('SET NAMES utf8');
$query = $pdo->query("SELECT name FROM TAMAGO_BORN WHERE num = 1");
$name100a = $query->fetch();
$name100 = $name100a[0];
		print<<<EOF
		<image src="image/sirudoraBorned.png" width="500px" height="300px" id="eggborn" usemap="#Egg" border="0"/><br><br>
EOF;
echo $name100."さんが１００回目のワラをかけ、シルドラの誕生を見届けました。<br>シルドラ「みんな、こんにちは。みんなのおかげで生まれることができたよ。これからよろしくね。今は産まれたばかりで疲れてるから、すこし休むね。」<br><br><br><a href=\"index.html\">地図へ戻る<a></text>";
exit;
	}
if(!($qjqj ==="ok") and !($_POST["disFlg"]==="true")){
print<<<EOF
<div id="kesitai">
<img src="gameimg/eggStop.png" width="500" height="300" id="before">
<br>
...
$eggPoint = $query->fetch();
	if($eggPoint[0] >= 100){
			print<<<EOF
<image src="gameimg/eggStop.png" width="500px" height="300px" id="eggborn" usemap="#Egg" border="0"/>
<map name="Egg">
...
$pdo = new PDO('mysql:dbname=LAA0535115-dbname;host=ホスト名', 'ユーザー名', 'パスワード');
$pdo->query('SET NAMES utf8');
$query = $pdo->query("SELECT name FROM db_user WHERE id = '$idlast'");
$namelast = $query->fetch();
$nanana = $namelast[0];
if(is_null($namelast[0])){
	$namelast[0]="ゲスト(非メンバー)";
}
echo "<text id=\"lastmsg\">".$namelast[0]."さんが１００回目のワラをかけました。<br>".$namelast[0]."さんには記念として、５シルドル<img src=\"image/sirudoru.png\">がプレゼントされます。<br><br>
ちなみにこのページはシルドラの誕生を見届けた".$namelast[0]."さんしか見ることができない<br>一度限りのページです。<br><a href=\"index.html\">地図へ戻る<a></text>";
$pdo = new PDO('mysql:dbname=LAA0535115-dbname;host=ホスト名', 'ユーザー名', 'パスワード');
$pdo->query('SET NAMES utf8');
$query = $pdo->query("INSERT INTO TAMAGO_BORN (num, name) VALUES (1,'$nanana')");
$query->execute();
$pdo = new PDO('mysql:dbname=LAA0535115-dbname;host=ホスト名', 'ユーザー名', 'パスワード');
$pdo->query('SET NAMES utf8');
$query2 = $pdo->query("UPDATE db_user SET sirudoru = sirudoru+5 WHERE name = '$nanana'");
$query2->execute();
exit;
	}
	//---------------------------------------------------
