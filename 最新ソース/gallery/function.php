...
function delDetaToImg($file_path,$max_line){
	$messe = '';
	$id=$_POST['id'];
	...
	if(@unlink("{$img_updir}/{$id}.{$extension_type}")) $messe .= "画像削除完了しました！ ";//メイン画像削除
	if(@unlink("{$img_updir}/thumb_{$id}.{$extension_type}")) $messe .= "サムネイル画像削除完了しました！ ";//サムネイル削除
	return "削除完了しました！";
}
